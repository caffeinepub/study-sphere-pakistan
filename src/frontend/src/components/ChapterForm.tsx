...
export default function ChapterForm({
  ...
  const [notesUrl3, setNotesUrl3] = useState(chapter?.notesUrl3 ?? "");
  const [notesLabel3, setNotesLabel3] = useState(chapter?.notesLabel3 ?? "");
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        const input = {
          ...
          notesUrl3: notesUrl3.trim(),
          notesLabel3: notesLabel3.trim(),
        ...
        if (isEditing && chapter) {
          const success = await callWithRetry(
            () =>
              actor.updateChapter(BigInt(chapter.id), input as ChapterInput),
            "Update chapter",
          );
          if (!success) throw new Error("Failed to update chapter");
          updateChapter.mutate(
            { id: chapter.id, input: input as ChapterInput },
            {
              onSuccess: () => {
                setIsSaving(false);
                onSuccess?.();
          ...
          await callWithRetry(
            () => actor.addChapter(input as ChapterInput),
            "Create chapter",
          );
          setIsSaving(false);
          onSuccess?.();
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-lg bg-muted/20 p-3 space-y-2">
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            <div>
              <Label htmlFor="notesLabel3" className="text-xs">
                Label (optional)
              </Label>
              <Input
                id="notesLabel3"
                value={notesLabel3}
                onChange={(e) => setNotesLabel3(e.target.value)}
                placeholder="e.g. Part 3, Supplementary"
              />
            </div>
            <div>
              <Label htmlFor="notesUrl3" className="text-xs">
                URL
              </Label>
              <Input
                id="notesUrl3"
                value={notesUrl3}
                onChange={(e) => setNotesUrl3(e.target.value)}
                placeholder="https://..."
                type="url"
              />
            </div>
