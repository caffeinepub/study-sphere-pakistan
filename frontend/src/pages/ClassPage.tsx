...
import { BookOpen, Microscope, FlaskConical, Zap, ChevronRight } from 'lucide-react';

const subjects = [
  { id: 'biology', label: 'Biology', icon: Microscope, color: 'bg-green-500 dark:bg-green-600', desc: 'Cell biology, genetics, ecology and more' },
  { id: 'chemistry', label: 'Chemistry', icon: FlaskConical, color: 'bg-blue-500 dark:bg-blue-600', desc: 'Organic, inorganic and physical chemistry' },
  { id: 'physics', label: 'Physics', icon: Zap, color: 'bg-yellow-500 dark:bg-yellow-600', desc: 'Mechanics, waves, electricity and optics' },
  { id: 'english', label: 'English', icon: BookOpen, color: 'bg-purple-500 dark:bg-purple-600', desc: 'Grammar, comprehension and literature' },
];
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 dark:bg-blue-900/30 text-primary dark:text-blue-400 text-sm font-medium mb-4">
            FSc Part {classNum === '11' ? '1' : classNum === '12' ? '2' : classNum}
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {subjects.map(subject => (
            <button
              key={subject.id}
              onClick={() => navigate({ to: '/class/$classNum/$subject', params: { classNum, subject: subject.id } })}
            ...
            >
              <div className={`w-14 h-14 rounded-xl ${subject.color} flex items-center justify-center flex-shrink-0 shadow-md`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{subject.desc}</p>
              </div>
