...
const subjects = [
  { label: 'Biology', icon: FlaskConical, path: '/mdcat/biology', color: 'text-green-600' },
  { label: 'Chemistry', icon: Atom, path: '/mdcat/chemistry', color: 'text-blue-600' },
  { label: 'Physics', icon: Brain, path: '/mdcat/physics', color: 'text-purple-600' },
  { label: 'English', icon: Languages, path: '/mdcat/english', color: 'text-orange-600' },
  { label: 'Logical Reasoning', icon: Brain, path: '/mdcat/logical-reasoning', color: 'text-pink-600' },
];
      ...
      <header className="bg-card border-b border-border sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Medical & Dental College Admission Test</p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-8">
        ...
        <section>
          ...
          {!isLoading && !isError && pastPapers.length === 0 && (
            <p className="text-muted-foreground text-sm py-4">No past papers added yet.</p>
          )}
            ...
            <div className="space-y-2">
              {pastPapers.map((entry) => (
                ...
                >
                  ...
                  <span className="flex-1 font-medium text-foreground text-sm">{entry.title}</span>
                  <a
                    href={entry.url}
                    target="_blank"
        ...
        <section>
          ...
          {!isLoading && !isError && practiceTests.length === 0 && (
            <p className="text-muted-foreground text-sm py-4">No practice tests added yet.</p>
          )}
