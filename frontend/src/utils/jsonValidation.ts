import type { QuizQuestion, FlashcardItem } from '../types/chapter';
...
export function validateQuizJSON(raw: string): ValidationResult {
  ...
  if (!Array.isArray(parsed)) {
    return { success: false, error: 'Quiz JSON must be an array of question objects.' };
  }
  for (let i = 0; i < parsed.length; i++) {
    ...
    if (typeof item.question !== 'string' || !item.question.trim()) {
      return { success: false, error: `Item at index ${i} must have a "question" string field.` };
    }
    if (!Array.isArray(item.options) || item.options.length < 2) {
      return { success: false, error: `Item at index ${i} must have an "options" array with at least 2 items.` };
    }
    for (let j = 0; j < item.options.length; j++) {
      if (typeof item.options[j] !== 'string') {
        return { success: false, error: `Item at index ${i}, option ${j} must be a string.` };
      }
    }
    if (typeof item.correctIndex !== 'number' || item.correctIndex < 0 || item.correctIndex >= item.options.length) {
      return { success: false, error: `Item at index ${i} must have a valid "correctIndex" number (0 to ${(item.options as unknown[]).length - 1}).` };
    }
  }
...
export function validateFlashcardJSON(raw: string): ValidationResult {
  ...
  if (!Array.isArray(parsed)) {
    return { success: false, error: 'Flashcard JSON must be an array of card objects.' };
  }
  for (let i = 0; i < parsed.length; i++) {
    const item = parsed[i] as Record<string, unknown>;
    if (typeof item !== 'object' || item === null) {
    ...
    if (typeof item.front !== 'string' || !item.front.trim()) {
      return { success: false, error: `Item at index ${i} must have a "front" string field.` };
    }
    if (typeof item.back !== 'string' || !item.back.trim()) {
      return { success: false, error: `Item at index ${i} must have a "back" string field.` };
    }
  }
